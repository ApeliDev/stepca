...
require_once 'usercontrollers/Deposit.php';
...
$deposits = $depositService->getDepositHistory($user_id);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['amount'])) {
    ...
    // Process deposit through the Deposit class
    $result = $depositService->initiateDeposit($user_id, $amount, $phone);
...
<head>
    ...
    <title><?php echo SITE_NAME; ?> - Deposit Funds</title>
            ...
            <main class="flex-1 p-4 lg:p-6 overflow-y-auto">
                ...
                <div class="bg-darker/60 backdrop-blur-sm rounded-2xl border border-primary/20 overflow-hidden animate-fadeIn">
                    <div class="p-6 border-b border-primary/20">
                        ...
                        <p class="mt-2 text-sm text-lightGray">Add funds to your account via M-Pesa</p>
                    </div>
                    ...
                    <div class="p-6">
                        ...
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 mb-8">
                            <div class="bg-darker/60 backdrop-blur-sm rounded-2xl border border-primary/20 p-6 hover:border-primary/40 transition-all duration-300 animate-scaleIn">
                                <div class="flex items-center">
                                    ...
                                    <div class="ml-5 flex-1">
                                        <dt class="text-sm font-medium text-lightGray">Current Balance</dt>
                            ...
                            <div class="bg-darker/60 backdrop-blur-sm rounded-2xl border border-primary/20 p-6 hover:border-primary/40 transition-all duration-300 animate-scaleIn" style="animation-delay: 0.1s;">
                                <div class="flex items-center">
                                    ...
                                    <div class="ml-5 flex-1">
                                        <dt class="text-sm font-medium text-lightGray">Minimum Deposit</dt>
                                        <dd class="flex items-baseline mt-1">
                                            <div class="text-2xl font-bold text-white">KES 100.00</div>
                                        </dd>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="animate-slideIn" style="animation-delay: 0.2s;">
                            <form id="deposit-form" method="POST" action="deposit.php" class="space-y-6">
                                ...
                                <div class="bg-green-500/10 border border-green-500/20 rounded-xl p-4 mb-6">
                                    <div class="flex items-center">
                                        ...
                                        <div class="ml-4">
                                            <p class="text-sm text-green-300"><?php echo htmlspecialchars($_SESSION['success_message']); ?></p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <p class="mt-2 text-xs text-lightGray/70">Minimum: KES 100</p>
                                </div>
                                ...
                                <div>
                                    ...
                                    <p class="mt-2 text-xs text-lightGray/70">
                                        Ensure your phone number is correct and has M-Pesa registered
                                    </p>
                                </div>
                            ...
                            <tbody class="divide-y divide-primary/10">
                                <?php if (count($deposits) > 0): ?>
                                    ...
                                    <tr class="hover:bg-primary/5 transition-colors animate-slideIn" style="animation-delay: <?php echo (0.4 + $index * 0.1); ?>s;">
                                        <td class="px-6 py-4 text-sm text-lightGray"><?php echo date('M j, Y H:i', strtotime($deposit['created_at'])); ?></td>
                                        <td class="px-6 py-4 text-sm text-white font-semibold">KES <?php echo number_format($deposit['amount'], 2); ?></td>
                                        <td class="px-6 py-4 text-sm text-lightGray"><?php echo htmlspecialchars($deposit['phone']); ?></td>
                                        <td class="px-6 py-4">
                                            <?php echo $depositService->getStatusBadge($deposit['status'], $deposit['mpesa_code']); ?>
                                        </td>
                                    </tr>
                                    <?php endforeach; ?>
                                <?php else: ?>
                                    <tr>
                                        <td colspan="4" class="px-6 py-8 text-center">
                                            <div class="flex flex-col items-center animate-fadeIn">
                                                ...
                                                <p class="text-lightGray text-sm">No deposits yet</p>
                                                <p class="text-lightGray/60 text-xs mt-1">Your deposit history will appear here</p>
                                            </div>
                                        </td>
                                    </tr>
                                <?php endif; ?>
