...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['sell_dollars'])) {
    $amount = $_POST['amount'];
    ...
    if (!$exchange_rate) {
        ...
        $_SESSION['error'] = "Minimum sell amount is $10";
    } else {
        // Calculate total KES to receive
        $total_kes = $amount * $exchange_rate['sell_rate'];
        
        // Create order
        $stmt = $conn->prepare("
            INSERT INTO orders 
            (user_id, order_type, currency_pair, amount, rate, total_amount, payment_method, wallet_address, status)
            VALUES (?, 'sell', 'USD/KES', ?, ?, ?, ?, ?, 'pending')
        ");
        $stmt->execute([$user_id, $amount, $exchange_rate['sell_rate'], $total_kes, $payout_method, $wallet_address]);
        
        $_SESSION['success'] = "Sell order for $".number_format($amount,2)." created successfully!";
...
<body class="bg-gradient-to-br from-dark via-darker to-darkest min-h-screen text-white">
        ...
        <div class="flex-1 flex flex-col min-h-screen">
            ...
            <main class="flex-1 p-4 lg:p-6 overflow-y-auto">
                <div class="flex justify-between items-center mb-6">
                    <h1 class="text-2xl font-bold text-white">Sell US Dollars</h1>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="lg:col-span-2 bg-darker/60 backdrop-blur-sm rounded-2xl border border-primary/20 p-6 animate-fadeIn">
                        ...
                        <form method="POST" action="">
                            ...
                            <div class="mb-6">
                                <label class="block text-lightGray text-sm mb-2">Payout Method</label>
                                <select 
                                    name="payout_method" 
                                    id="payoutMethod"
                                    class="w-full px-4 py-3 bg-darker border border-primary/20 rounded-lg focus:outline-none focus:border-primary/50"
                            ...
                            <div class="mb-6 hidden" id="walletAddressField">
                            ...
                            <div class="mb-6 bg-darker/50 border border-primary/20 rounded-lg p-4">
                                ...
                                <div class="flex justify-between text-primary font-semibold">
                                    ...
                                    <span id="totalReceive">KES 0.00</span>
                                </div>
                            </div>
                    ...
                    <div class="bg-darker/60 backdrop-blur-sm rounded-2xl border border-primary/20 p-6 animate-fadeIn" style="animation-delay: 0.1s;">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <div class="bg-darker/50 border border-primary/20 rounded-lg p-3 space-y-2">
                                    <div class="flex items-start">
                                        ...
                                        <span>Enter the USD amount you want to sell</span>
                                    </div>
                                    <div class="flex items-start">
                            ...
                            <div>
                                ...
                                <div class="bg-darker/50 border border-primary/20 rounded-lg p-3">
                                    <p>USD sales are processed within <span class="font-semibold text-primary">1-2 business hours</span> after order confirmation.</p>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        ...
        const exchangeRate = <?php echo $exchange_rate ? $exchange_rate['sell_rate'] : 0; ?>;
        document.querySelector('input[name="amount"]').addEventListener('input', function(e) {
            ...
            document.getElementById('totalReceive').textContent = 'KES ' + kesAmount.toFixed(2);
        });
        
        // Show/hide wallet address field based on payout method
        document.getElementById('payoutMethod').addEventListener('change', function(e) {
            const walletField = document.getElementById('walletAddressField');
            if (e.target.value === 'crypto_wallet') {
                walletField.classList.remove('hidden');
                walletField.querySelector('input').required = true;
            } else {
                walletField.classList.add('hidden');
                walletField.querySelector('input').required = false;
            }
        });
    </script>
